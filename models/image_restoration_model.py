...
class Kizz(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(Kizz, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        preds,L1,L2,L3,L4,R1,R2 = self.net_g(self.lq,self.gt)
        ...
        l_pix = 0.
        for pred in preds:
            l_pix += self.cri_pix(L2,R1,pred, self.gt,R2,L4,L3,L1) #此处统计batch的loss
    ...
    # Run the model and get only the first output (predicted image)
        self.nonpad_test(img)
        # if isinstance(self.output, tuple):
        #    self.output = self.output[0]  # Extract the first output

    # Get the size of the predicted output
        _, _, h, w = self.output.size()
    ...
    def nonpad_test(self, img=None):
        ...
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                pred,L1,L2,L3,L4,R1,R2 = self.net_g_ema(img,self.gt)
            ...
            self.net_g.eval()
            with torch.no_grad():
                pred,L1,L2,L3,L4,R1,R2 = self.net_g(img,self.gt)
            if isinstance(pred, list):
                pred = pred[-1]
            self.output = pred
            self.net_g.train()
    ...
    #         self.net_g_ema.eval()
    #         with torch.no_grad():
    #             pred = self.net_g_ema(img, self.gt)  # 传递 img 和 gt
    #         if isinstance(pred, tuple):  # 如果返回了一个 tuple
    #             pred = pred[0]  # 只取预测输出
    #         self.output = pred
    #     else:
    #         self.net_g.eval()
    #         with torch.no_grad():
    #             pred = self.net_g(img, self.gt)  # 传递 img 和 gt
    #         if isinstance(pred, tuple):  # 如果返回了一个 tuple
    #             pred = pred[0]  # 只取预测输出
    #         self.output = pred
    #         self.net_g.train()
